...
void particle::update(const float dt) {
	...
	if(m_lifeSpan != 0 && m_startColour != m_endColour) {
		float norm = 1.0f - clamp(m_life / m_lifeSpan, 0.0f, 1.0f);
...
particleStripped particle::toStripped() const {
	return { m_M,m_colour,m_texture,m_life };
}

void particle::updateStripped(particleStripped& in) const {
	in.m_M = m_M;
	in.m_colour = m_colour;
	in.m_life = m_life;
	in.m_texture = m_texture;
}
